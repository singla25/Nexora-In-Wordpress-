...
use BetterMessages\GuzzleHttp\Cookie\CookieJar;
use BetterMessages\GuzzleHttp\Exception\GuzzleException;
use BetterMessages\GuzzleHttp\Exception\InvalidArgumentException;
 ...
 * @final
 */
class Client implements ClientInterface, \BetterMessages\Psr\Http\Client\ClientInterface
{
    use ClientTrait;
    /**
    ...
    public function __construct(array $config = [])
    {
        ...
        if (isset($config['base_uri'])) {
            $config['base_uri'] = \BetterMessages\GuzzleHttp\Psr7\Utils::uriFor($config['base_uri']);
        }
    ...
    public function requestAsync(string $method, $uri = '', array $options = []): PromiseInterface
    {
        ...
        $uri = $this->buildUri(\BetterMessages\GuzzleHttp\Psr7\Utils::uriFor($uri), $options);
        if (\is_array($body)) {
    ...
    {
        if (isset($config['base_uri'])) {
            $uri = \BetterMessages\GuzzleHttp\Psr7\UriResolver::resolve(\BetterMessages\GuzzleHttp\Psr7\Utils::uriFor($config['base_uri']), $uri);
        }
    ...
    private function applyOptions(RequestInterface $request, array &$options): RequestInterface
    {
        ...
        if (isset($options['form_params'])) {
            ...
            $options['_conditional'] = \BetterMessages\GuzzleHttp\Psr7\Utils::caselessRemove(['Content-Type'], $options['_conditional']);
        ...
        if (!empty($options['decode_content']) && $options['decode_content'] !== true) {
            // Ensure that we don't have the header in different case and set the new value.
            $options['_conditional'] = \BetterMessages\GuzzleHttp\Psr7\Utils::caselessRemove(['Accept-Encoding'], $options['_conditional']);
        ...
        if (isset($options['body'])) {
            ...
            $modify['body'] = \BetterMessages\GuzzleHttp\Psr7\Utils::streamFor($options['body']);
            unset($options['body']);
        }
        if (!empty($options['auth']) && \is_array($options['auth'])) {
            ...
            switch ($type) {
                case 'basic':
                    // Ensure that we don't have the header in different case and set the new value.
                    $modify['set_headers'] = \BetterMessages\GuzzleHttp\Psr7\Utils::caselessRemove(['Authorization'], $modify['set_headers']);
        ...
        $request = \BetterMessages\GuzzleHttp\Psr7\Utils::modifyRequest($request, $modify);
        if ($request->getBody() instanceof \BetterMessages\GuzzleHttp\Psr7\MultipartStream) {
