...
use BetterMessages\GuzzleHttp\Exception\ConnectException;
use BetterMessages\GuzzleHttp\Exception\RequestException;
use BetterMessages\GuzzleHttp\Promise as P;
use BetterMessages\GuzzleHttp\Promise\FulfilledPromise;
use BetterMessages\GuzzleHttp\Promise\PromiseInterface;
use BetterMessages\GuzzleHttp\Psr7\LazyOpenStream;
use BetterMessages\GuzzleHttp\TransferStats;
use BetterMessages\GuzzleHttp\Utils;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \BetterMessages\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri): string
    {
        ...
        $redactedUriString = \BetterMessages\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \BetterMessages\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \BetterMessages\GuzzleHttp\Psr7\Utils::streamFor($sink);
